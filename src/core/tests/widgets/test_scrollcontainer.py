...
class ScrollContainerTests(TestCase):
    ...
    def test_assign_to_window_without_content(self):
        # 1. preconditions: sc has a value and window is None
        # 2. set values and assert it
        ...
        # error is raised when setting window to empty ScrollContainer
        self.sc = None
        with self.assertRaises(AttributeError):
            self.sc.window = toga.window

    def test_assign_to_window_with_content(self):
