...
class TestCase(unittest.TestCase):
    ...
    def assertValueSet(self, _instance, attr, value):
        ...
        try:
            ...
            self.fail('Widget {} did not have the attribute {!r} set; set attributes were {}.'.format(
                _instance,
                attr,
                ', '.join('{!r}'.format(a) for a in sorted(_instance._impl._sets.keys()))
            ))
        except AttributeError:
            self.fail('Widget {} is not a logged object'.format(_instance))

    def assertValuesSet(self, _instance, attr, values):
    ...
    def assertValueGet(self, _instance, attr):
        ...
        try:
            self.assertIn(
                attr,
                _instance._impl._gets,
                'Widget {} did not retrieve the attribute {!r}; retrieved attributes were {}.'.format(
                    _instance,
                    attr,
                    ','.join('{!r}'.format(a) for a in sorted(_instance._impl._gets))
            ))
        except AttributeError:
            self.fail('Widget {} is not a logged object'.format(_instance))

    def assertActionNotPerformed(self, _instance, action):
        """Assert that the named action was *not* performed by a widget.

        Args:
            _instance: The interface of the widget that should not have performed the action.
        ...
        try:
            self.assertNotIn(
                action,
                _instance._impl._actions,
                'Action {} unexpectedly performed by {}.'.format(action, _instance)
            )
        except AttributeError:
            self.fail('Widget {} is not a logged object'.format(_instance))

    def assertActionPerformed(self, _instance, action):
        """Assert that the named action performed by a widget.

        Args:
            _instance: The interface of the widget that should have performed the action.
            action: The name of the action to check
        ...
        try:
            self.assertIn(
                ...
                'Action {} not performed by {}. Actions were: {}'.format(
                    action,
                    _instance,
                    sorted(_instance._impl._actions.keys())
                )
            )
        except AttributeError:
            self.fail('Widget {} is not a logged object'.format(_instance))

    def assertActionPerformedWith(self, _instance, action, **test_data):
        ...
        try:
            ...
            # this object.
            for sequence, data in _instance._impl._actions[action].items():
            ...
            self.fail('Action {} not performed by {} with {}. Actions were: {}'.format(
                    ...
                    _instance,
                    test_data,
                    sorted(_instance._impl._actions[action].items())
                ))
        except KeyError:
