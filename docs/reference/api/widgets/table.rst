...
* If the value is a :any:`tuple`, it must have two elements: an :any:`Icon` which will
  be displayed on the left of the cell, and a second element which will be interpreted
  as below.
...
* Any other value will be converted into a string. If an icon has not already been
  provided in a tuple (above), and the value has an ``icon`` attribute which is not
  ``None``, that icon will be displayed on the left of the cell.
