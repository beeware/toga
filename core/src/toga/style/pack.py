...
class Pack(BaseStyle):
    ...
    def _layout_node(self, node, alloc_width, alloc_height, scale):
        ...
        if self.width != NONE:
            ...
            if node.intrinsic.width is not None:
                ...
                # self._debug(f"USE ALL WIDTH {available_width=}")
                pass
        ...
        if self.height != NONE:
            ...
            if node.intrinsic.height is not None:
                ...
                # self._debug("USE ALL HEIGHT {available_height=})
                pass
    ...
    def _layout_row_children(self, node, available_width, available_height, scale):
        ...
        if full_flex > 0 and remaining_width is not None:
    ...
    def _layout_column_children(self, node, available_width, available_height, scale):
        ...
        if full_flex > 0 and remaining_height is not None:
