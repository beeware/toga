...
def test_widget_created_with_values(content, on_scroll_handler):
    ...
    assert_action_performed_with(scroll_container, "set content", widget=content._impl)
...
def test_assign_to_app_no_content(app):
    """If the widget is assigned to an app, and there is no content, nothing happens"""
    scroll_container = toga.ScrollContainer()
...
def test_content(app, window, scroll_container, content):
    """The content of the scroll container can be changed"""
    # Assign the scroll container to an app and window
    scroll_container.app = app
    scroll_container.window = window
...
def test_get_horizontal_position_when_not_horizontal(scroll_container):
    ...
    with pytest.raises(
        ValueError,
        match=r"Cannot get horizontal position when horizontal is not set.",
    ):
        scroll_container.horizontal_position


def test_set_horizontal_position_when_not_horizontal(scroll_container):
...
def test_get_vertical_position_when_not_vertical(scroll_container):
    ...
    with pytest.raises(
        ValueError,
        match=r"Cannot get vertical position when vertical is not set.",
    ):
        scroll_container.vertical_position
