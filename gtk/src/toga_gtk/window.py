...
class Window:
    def __init__(self, interface, title, position, size):
        ...
        pos = 100 + len(toga.App.app.windows) * 50
    ...
    def close(self):
        # Disconnect the delete handler so that we can perform the actual close.
...
class MainWindow(Window):
    ...
    def create_toolbar(self):
        # If there's an existing toolbar, remove it until we know we need it.
        ...
        if self.toolbar_items:
            ...
            self.layout.pack_start(
                self.native_toolbar, expand=False, fill=False, padding=0
            )
