...
class ObjCClass(object):
    ...
    def __new__(cls, class_name_or_ptr):
        ...
        objc_class.properties = {}         # mapping of name -> true/false for existence of property
    ...
    def has_property(self, name):
        """Evaluate if the class has the given property. This is done by looking
        for the write selector (setPropertyName:)."""
        try:
...
class ObjCInstance(object):
    ...
    def __repr__(self):
        if self.objc_class.name == b'NSCFString':
            ...
            from .core_foundation import cfstring_to_string
    ...
    def __getattr__(self, name):
        ...
        if self.objc_class.has_property(name):
            from .core_foundation import get_NSString
            return self.valueForKey_(get_NSString(name))
    ...
    def __setattr__(self, name, value):
        ...
        if self.objc_class.has_property(name):
            from .core_foundation import get_NSString
            self.setValue_forKey_(value, get_NSString(name))
            return
        raise AttributeError('ObjCInstance %s cannot set attribute %s' % (self.objc_class.name, name))
