...
async def test_title(second_window, second_window_probe):
    """The title of a simple window can be changed"""
    original_title = second_window.title
    assert original_title == "Toga"
    await second_window_probe.wait_for_window("Window title can be retrieved")

    try:
        second_window.title = "A Different Title"
        assert second_window.title == "A Different Title"
        await second_window_probe.wait_for_window("Window title can be changed")
    finally:
        second_window.title = original_title
        assert second_window.title == "Toga"
        await second_window_probe.wait_for_window("Window title can be reverted")
...
if toga.platform.current_platform in {"iOS", "android"}:
    ...
    @pytest.mark.parametrize("second_window_kwargs", [{}])
    async def test_secondary_window(
        app, second_window, second_window_probe, default_window_title
    ):
        """A secondary window can be created"""
        assert second_window.app == app
        assert second_window in app.windows

        assert second_window.title == default_window_title
        assert second_window.size == (640, 480)
        # Position will be cascaded; the exact position depends on the platform,
        # and how many windows have been created. As long as it's not (100,100),
        # and the x an y coordinate are the same
        assert second_window.position[0] > 110
        assert second_window.position[1] == second_window.position[0]
    ...
    async def test_secondary_window_toolbar(app, second_window, second_window_probe):
        """Simple windows don't have toolbars"""
