...
async def test_enabled(widget, probe):
    ...
    await probe.redraw(message="Widget should be disabled")
    ...
    # Enable the widget
    widget.enabled = True
    await probe.redraw(message="Widget should be enabled")
...
async def test_focus(widget, probe):
    ...
    other.focus()
    await probe.redraw(message="A separate widget should be given focus")
    assert not probe.has_focus
    assert other_probe.has_focus

    widget.focus()
    await probe.redraw(message="Widget should be given focus")
...
async def test_text(widget, probe):
    ...
    for text in TEXTS:
        widget.text = text
        await probe.redraw(message="Widget text should be %s" % text)
...
async def test_text_width_change(widget, probe):
    ...
    await probe.redraw(message="Widget text should be very long")
...
async def test_font(widget, probe):
    ...
    await probe.redraw(message="Widget font should be larger than its original size")
    ...
    await probe.redraw(message="Widget font should be changed to FANTASY")
...
async def test_color(widget, probe):
    ...
    for color in COLORS:
        ...
        await probe.redraw(message="Widget text color should be %s" % color)
...
async def test_color_reset(widget, probe):
    ...
    await probe.redraw(message="Widget text color should be RED")
    ...
    await probe.redraw(message="Widget text color should be restored to the original")
...
async def test_background_color(widget, probe):
    ...
    for color in COLORS:
        ...
        await probe.redraw(message="Widget text background color should be %s" % color)
...
async def test_background_color_reset(widget, probe):
    ...
    await probe.redraw(message="Widget text background color should be RED")
...
async def test_background_color_transparent(widget, probe):
    ...
    await probe.redraw(message="Widget text background color should be TRANSPARENT")
...
async def test_flex_widget_size(widget, probe):
    ...
    widget.style.background_color = RED
    await probe.redraw(message="Widget background color should be RED")
    ...
    await probe.redraw(message="Widget should be 100px wide now")
    assert probe.width == approx(100, rel=0.01)
    ...
    await probe.redraw(message="Widget should be the size of container now")
    ...
    await probe.redraw(message="Widget should be reverted to fixed height")
    assert probe.width > 300
    ...
    await probe.redraw(message="Widget should be reverted to fixed width")
...
async def test_flex_horizontal_widget_size(widget, probe):
    ...
    await probe.redraw(message="Widget should be a non-flex row box")
    ...
    await probe.redraw(message="Widget should be changed to 300px width x 200px height")
