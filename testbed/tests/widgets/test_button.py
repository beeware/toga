...
async def test_text(widget, probe):
    ...
    for text in TEXTS:
        widget.text = text
        await probe.redraw(message="Button text should be %s" % text)
...
async def test_press(widget, probe):
    ...
    await probe.press()
    await probe.redraw(message="Button should be pressed")
...
async def test_background_color_transparent(widget, probe):
    ...
    await probe.redraw(message="Button background color should be transparent")
