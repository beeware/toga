...
def build(app):
        ...
        def choose(pick_heads):
            buttons[0].enabled, buttons[1].enabled = (not pick_heads, pick_heads) #TODO allow to pick either instead of disable

            coins[0].image, coins[1].image, announce.text = (TOSS, TOSS, "Come in spinner")
            yield 2

            for coin in coins: coin.image = random.choice([HEADS, TAILS])
